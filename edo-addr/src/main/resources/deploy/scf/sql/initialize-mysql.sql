...
CREATE TABLE SYS_KEYS(
    ...
    S_COLUMNS TEXT NOT NULL,                    -- 列信息，Json格式，对应字段名（SYS_FIELDS）的集合
    -- 关联属性
...
CREATE TABLE SYS_VIEW(
    ...
    D_STATEMENT TEXT NOT NULL,                  -- SQL或NON-SQL语句
...
CREATE TABLE SYS_INDEX(
    ...
    D_COLUMNS TEXT NOT NULL,                    -- JSON格式：[{"column":"A","mode":"ASC"}]
...
CREATE TABLE EVX_URI(
    ...
    S_MAPPING TEXT,                             -- 参数映射表
    S_JOINED TEXT,                                 -- 执行Joined查询以及删专用Api信息，处理DELETE和INSERT
    L_REQUIRED_PARAM TEXT,                      -- 必须参数列表,
    L_RETURN_FILTERS TEXT,                      -- 返回结果中需要过滤的属性值
    L_CONTENT_MIMES TEXT,                       -- Content Type中对应MIME
    L_ACCEPT_MIMES TEXT,                        -- Accept Type中对应MIME
    PRIMARY KEY(K_ID)
);
-- EVX_URI的索引创建
...
CREATE TABLE EVX_RULE(
    ...
    J_CONFIG TEXT,                              -- Java组件中需要使用的Json配置
...
CREATE TABLE EVX_SOCK(
    ...
    J_PERMITS TEXT,                               -- Sock中当前处理器的Permit配置
    J_CONFIG TEXT,                                -- Sock中其他配置
